# 存在bug：送给vl的图片是原图，正确的应该是处理后的图。修复后的代码在 newtest0318中
...
def perform_ocr(ocr_result_label, ocr_prompt, main_window):
    try:
        if ocr_prompt.text():
            ...
            import base64
            with open(main_window.original_img_path, "rb") as f:
                image_data = f.read()
            ...
            # ocr_result_label.setText("A） 从长远来看，人类与森林的关系一直是野蛮的破坏，但即使是这种关系也带来了缓慢的希望。在中世纪，世界上大部分地区都不缺乏木材，很少有人认为砍伐森林是一个问题。然而，1548年，威尼斯人估计，以目前的使用速度，重要的木材供应只能持续30年，但不同的森林管理将使满足几个世纪的需求成为可能。保护资源的想法源于对未来的担忧：担心资源的消耗速度超过其补充速度。B）经济利益是这种对树木和森林理解的核心。三个多世纪后，科学家们才开始认识到木材生产不是森林的唯一功能，也可能不是最重要的功能。19世纪末和20世纪初，人们越来越认识到森林是相互依存的国家动植物物种的栖息地。它们是否接管了防止土壤侵蚀和滑坡的保护功能？h对水平衡做出重大贡献，因为它们可以防止地表径流、过滤空气中的颗粒物、温室气体和放射性物质；它们产生氧气；它们为繁殖提供了空间，并保存了氯和史前遗迹。因此，世界各地的森林被划为公园或荒野地区。")
...
class ControlWindow(QMainWindow):
    def __init__(self, main_window):
        ...
        # button_layout.addWidget(self.model_combo_vl)
